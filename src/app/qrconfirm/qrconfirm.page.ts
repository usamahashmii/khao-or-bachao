...
@Component({
  selector: 'app-qrconfirm',
  templateUrl: './qrconfirm.page.html',
  styleUrls: ['./qrconfirm.page.scss'],
})
export class QrconfirmPage implements OnInit {
    ...
    if(this.scannedCode){
      ...
      this.restService.confirmReservationForUser(stringy).subscribe(response => {
        ...
        if(JSON.parse(response['_body']).status == 'error'){
          ...
          this.toastService.presentToastMessage('Congratulations! Your reservation is confirmed!');
      ...
      this.toastService.presentToastMessage('Please scan the Qr Code to proceed!');
    }
  }
  ...
  ionViewWillEnter(){
    ...
    this.route.queryParams.subscribe(data =>{
      ...
      this.restService.getSingleRestaurantDetails(stringy).subscribe(data => {
        ...
        this.privacyPolicy = JSON.parse(data['_body']).restaurant_data[0].privacy_policy;
